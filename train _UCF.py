...
def main():
    ...
    args.seed = time.time()
    args.print_freq = 30
    args.train_json = './json/UCF_data1_train.json'
    args.test_json= './json/UCF_data1_test.json'
    args.gpu = '0'
    args.task = 'UCF_data1'
    # args.pre = './pre-trianed model/UCF_data1model_best.pth.tar'
...
def validate(val_list, model, criterion):
    ...
    mse = np.sqrt(mse.data.cpu().numpy() / len(test_loader))
    print(' * MAE {mae:.3f} '
          .format(mae=mae))
    print(' * MSE {mse:.3f} '
          .format(mse=mse))
